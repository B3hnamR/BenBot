...
def _format_products_text(products: Sequence) -> str:
    ...
    for index, product in enumerate(products, start=1):
        status = "ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Active" if product.is_active else "ÃƒÂ°Ã…Â¸Ã…Â¡Ã‚Â« Inactive"
        lines.append(
            f"{index}. {html.escape(product.name)} ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â {status}\n"
...
def _format_product_details(product) -> str:
    summary = html.escape(product.summary) if product.summary else "ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â"
    description = html.escape(product.description) if product.description else "ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â"
...
def _format_questions_text(questions: Iterable) -> str:
    ...
    for index, question in enumerate(questions, start=1):
        required = "ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ Required" if question.is_required else "ÃƒÂ¢Ã…Â¾Ã¢â‚¬â€œ Optional"
        base = (
            f"{index}. <code>{html.escape(question.field_key)}</code>"
            f" ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â {question.question_type.value} ({required})"
        )
...
async def render_questions_overview(
    ...
    *,
    ...
    header = f"<b>{html.escape(product.name)}</b> ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â form builder\n\n"
    payload = header + text
...
@router.callback_query(ProductAdminCallback.filter(F.action == "add"), ~StateFilter(ProductCreateState))
...
async def start_question_creation(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await callback.message.answer(
        "Enter a field key for this question.\nUse lowercase letters, digits and underscores."
...
async def confirm_product_creation(callback: CallbackQuery, state: FSMContext, session: AsyncSession) -> None:
    if await state.get_state() != ProductCreateState.confirm.state:
        await callback.answer()
        return
...
async def confirm_question_creation(callback: CallbackQuery, state: FSMContext, session: AsyncSession) -> None:
...
async def set_question_required(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    if question_type in {ProductQuestionType.SELECT, ProductQuestionType.MULTISELECT}:
        ...
        await callback.message.edit_text(
            "Provide answer options separated by commas.",
        )
    else:
...
async def collect_product_position(message: Message, state: FSMContext) -> None:
    ...
    text = (
        ...
        f"Summary: {html.escape(data.get('summary') or 'ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â')}\n"
        f"Description: {html.escape(data.get('description') or 'ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â')}"
    )
...
async def _cancel_state(message: Message, state: FSMContext, text: str) -> None:
    await state.clear()
    await message.answer(text)


@router.message(
    StateFilter(ProductCreateState, ProductEditState, ProductQuestionCreateState),
    Command("cancel"),
)
async def cancel_via_command(message: Message, state: FSMContext) -> None:
    await _cancel_state(message, state, "Operation cancelled.")

def _field_prompt(field: str) -> str:
    if field == "name":
        return "Send new name for the product.\nSend /cancel to abort."
    if field == "summary":
        return "Send new summary text. Use /skip to clear."
    if field == "description":
        return "Send new description text. Use /skip to clear."
    if field == "price":
        return "Send new price (decimal number)."
    if field == "currency":
        return "Send new currency code (3 letters)."
    if field == "inventory":
        return "Send inventory quantity (integer). Use /skip for unlimited."
    if field == "position":
        return "Send new display position (integer)."
    return "Provide new value."
...
async def apply_field_edit(message: Message, state: FSMContext, session: AsyncSession) -> None:
    ...
    service = ProductAdminService(session)
    update_payload = {field: parsed_value}
    if cleared:
...
def _parse_edit_value(field: str, raw_value: str) -> tuple[Any, bool]:
    ...
    if field in {"summary", "description"}:
        if _is_skip_message_value(value):
            return None, True
        if not value:
            raise ProductValidationError("Value cannot be empty. Use /skip to clear.")
        return value, False
    if field == "price":
        return _parse_decimal(value), False
    if field == "currency":
        currency = value.upper()
        if not re.fullmatch(r"[A-Z]{3}", currency):
            raise ProductValidationError("Currency must be a 3-letter ISO code.")
        return currency, False
    if field == "inventory":
        if _is_skip_message_value(value):
            return None, True
        return _parse_integer(value, allow_zero=True), False
    if field == "position":
        return _parse_integer(value, allow_zero=False), False
    raise ProductValidationError("Unsupported field.")


def _is_skip_message_value(value: str) -> bool:
    return value.lower() in {"/skip", "skip"}
